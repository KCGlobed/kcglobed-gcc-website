export default defineEventHandler(async (event) => {
    try {
        ...
        const isSuccess = body.errorName?.toUpperCase() === 'SUCCESS' || body.errorName?.toUpperCase() === 'INFO';
        if (isSuccess) {
            console.log("====== CLIENT SIDE SUCCESS/EVENT LOGGED ON SERVER ======");
            console.log(`Time: ${timestamp}`);
            console.log(`Context: ${body.context || 'Unknown context'}`);
            console.log(`Event Name: ${body.errorName || 'Unknown'}`);
            console.log(`Event Data: ${typeof body.errorData === 'object' ? JSON.stringify(body.errorData) : body.errorData || 'None'}`);
            console.log(`User Agent: ${body.userAgent || 'Unknown'}`);
            console.log(`User Info: ${JSON.stringify(body.userInfo) || 'None'}`);
            console.log("=======================================================");
        } else {
